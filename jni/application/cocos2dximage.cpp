...
namespace gcn
{
    COCOS2DXImage::COCOS2DXImage(SDLMOD_Surface* surface, bool autoFree)
    {
        mAutoFree = autoFree;
        mSurface = surface;
    }

    COCOS2DXImage::~COCOS2DXImage()
    {
        if (mAutoFree)
        {
            free();
    ...
    SDLMOD_Surface* COCOS2DXImage::getSurface() const
    {
        return mSurface;
    }

    int COCOS2DXImage::getWidth() const
    {
        if (mSurface == NULL)
        {
    ...
    int COCOS2DXImage::getHeight() const
    {
        if (mSurface == NULL)
        {
    ...
    Color COCOS2DXImage::getPixel(int x, int y)
    {	
		if (mSurface == NULL)
        {
    ...
    void COCOS2DXImage::putPixel(int x, int y, const Color& color)
    {
        if (mSurface == NULL)
        {
    ...
    void COCOS2DXImage::convertToDisplayFormat()
    {
        if (mSurface == NULL)
        {
