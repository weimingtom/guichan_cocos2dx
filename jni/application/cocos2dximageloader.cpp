...
namespace gcn
{
    Image* COCOS2DXImageLoader::load(const std::string& filename,
                                bool convertToDisplayFormat)
    {
        ...
        Image *image = new COCOS2DXImage(surface, true);
    ...
    SDLMOD_Surface* COCOS2DXImageLoader::loadSDLSurface(const std::string& filename)
    {
		...
		if (texture_data2)
		{
			...
			for (j = 0; j < g_TextureHeight2; ++j)
			{
				for (i = 0; i < g_TextureWidth2; ++i)
				{
					...
					{
						fprintf(stderr, "[COCOS2DXImageLoader::loadSDLSurface] hit (%d, %d) (%X, %X, %X)\n",
							i, j, r, g, b);
					}
#endif
