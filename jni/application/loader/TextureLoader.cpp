...
static CCFILE *ccfopen(const char *pszFile, const char *mode)
{
	CCFILE *result;
	result = (CCFILE *)malloc(sizeof(CCFILE));
	result->pBuffer = (char*)cocos2d::CCFileUtils::sharedFileUtils()->getFileData(pszFile, mode, &result->size);
	result->pos = 0;
	return result;
}

#undef fread
#define fread ccfread
static size_t ccfread(void *buffer, size_t size, size_t count, CCFILE *stream)
{
	size_t result = 0;
	if (stream->pos >= 0 && stream->size > stream->pos)
	{
		size_t si = size * count;
		if (stream->pos + si > stream->size)
		{
			si = stream->size - stream->pos;
		}
		memcpy(buffer, stream->pBuffer + stream->pos, si);
		stream->pos += si;
		result = si;
	}
	else
...
#undef fclose
#define fclose ccfclose
static int fclose( CCFILE *fp )
{
	if (fp == NULL)
	{
		return -1;
	...
	int flipY, int flipToBGR)
{
	...
	name = cocos2d::CCFileUtils::sharedFileUtils()->fullPathForFilename(name.c_str());
	filename = name.c_str();

	//cocos2d::CCLog("====>%s", filename);
#endif
